...
import { Plus, Package, Tag, BarChart3, Clock, Loader2, Trash2, Power } from "lucide-react";
...
function Admin() {
  ...
  const reload = async () => {
    try {
      ...
      toast.error("Erro ao carregar dados da API. Verifique se o backend está rodando.");
      console.error(e);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 p-4 sm:p-6">
      <div>
        ...
        <h1 className="mt-1 font-display text-3xl font-bold tracking-tight">{t("admin.title")}</h1>
        <p className="mt-1 text-sm text-muted-foreground">{t("admin.sub")}</p>
      </div>

      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <StatCard label="Anúncios monitorados" value={String(stats?.total ?? 0)} icon={Package} accent="primary" />
        <StatCard label="Termos ativos" value={String(watchlist.filter((w) => w.ativo).length)} icon={Tag} />
        <StatCard label="Vendidos" value={String(stats?.fechados ?? 0)} icon={BarChart3} accent="gold" />
        <StatCard
          label="Preço médio venda"
          value={stats?.preco_medio_venda ? formatPrice(stats.preco_medio_venda) : "—"}
          icon={Clock}
          hint={stats?.ultima_venda ? `Última: ${new Date(stats.ultima_venda).toLocaleDateString("pt-BR")}` : undefined}
        />
      </div>
                ...
                {(stats?.vendas_recentes ?? []).map((v) => (
                  <tr key={v.mlb_id} className="hover:bg-muted/20">
                    ...
                    <td className="px-4 py-2.5 num font-semibold text-primary">
                      {v.preco_final ? formatPrice(v.preco_final) : "—"}
                    </td>
                    <td className="px-4 py-2.5 text-right text-xs text-muted-foreground">
                      {v.finalizado_em ? <TimeAgo iso={v.finalizado_em} /> : "—"}
                    </td>
                  </tr>
                ))}
...
function AddTermForm({ onAdd }: { onAdd: () => void }) {
  ...
  const handleAdd = async () => {
    if (!termo.trim()) { toast.error("Digite um termo para monitorar."); return; }
    setSaving(true);
    try {
      ...
      setTermo("");
      onAdd();
    } catch { toast.error("Erro ao adicionar termo."); }
...
function WatchlistRow({ query, onRefresh }: { query: SearchQuery; onRefresh: () => void }) {
  ...
  const handleToggle = async () => {
    ...
    try {
      ...
      onRefresh();
    } catch { toast.error("Erro ao atualizar termo."); }
    finally { setLoading(false); }
  };

  const handleDelete = async () => {
    if (!confirm(`Remover "${query.termo}"?`)) return;
    setLoading(true);
    try {
      await deleteWatchlistTerm(query.id);
      toast.success("Termo removido.");
      onRefresh();
    } catch { toast.error("Erro ao remover termo."); }
    finally { setLoading(false); }
  };
  ...
  return (
    <tr className="hover:bg-muted/20">
      ...
      <td className="px-4 py-2.5 text-muted-foreground">
        ...
        <span className="ml-1 text-xs">({query.ativos} ativos · {query.fechados} vendidos)</span>
      </td>
      <td className="px-4 py-2.5 text-xs text-muted-foreground hidden md:table-cell">
        {query.ultima_busca ? <TimeAgo iso={query.ultima_busca} /> : "Nunca"}
      </td>
      <td className="px-4 py-2.5">
        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${query.ativo ? "bg-success/10 text-success" : "bg-muted text-muted-foreground"}`}>
          {query.ativo ? "Ativo" : "Pausado"}
        </span>
      </td>
      <td className="px-4 py-2.5 text-right">
        <Button variant="ghost" size="sm" disabled={loading} onClick={handleToggle} title={query.ativo ? "Pausar" : "Reativar"}>
          <Power className="h-3.5 w-3.5" />
        </Button>
        <Button variant="ghost" size="sm" disabled={loading} onClick={handleDelete} title="Remover" className="text-destructive hover:text-destructive">
